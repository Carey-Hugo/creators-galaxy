...
| 排名 | 成员 | 消息数 | 贡献分 | 覆盖群 |
|------|------|--------|--------|--------|
| 1 | Hugo | 159 | 205.0 | 创客星球 MVP 黑客松 + Hugo一人公司 |
...
|------|--------|--------|---------|---------|------|
| Hugo | 159 | 205.0 | 05-29 14:16 | 06-04 19:58 | — |
...
|------|---------|--------|----------|---------|
| 创客星球 MVP 黑客松 | -1003916141713 | supergroup | administrator | ✅ 完整归档（613 条）|
| Hugo一人公司 | -5076629166 | group | administrator | ⚠️ 仅 bot 自身消息（99 条，5/29）|
| 创客星球CGHub | -5223347644 | group | administrator | ❌ 0 条（隐私模式阻挡）|
| AI x Web3 School | -5291819613 | group | member | ❌ 0 条（隐私模式阻挡）|

**根因**：老式 `group` 在 Telegram 隐私模式下，bot 只能看到 @ 自己的消息，无法 forward/copy 普通消息。`supergroup`（创客星球 MVP 黑客松）不受此限制。

**修复路径（Hugo 决策）**：
1. 把 3 个 group 升级为 supergroup（群设置 → 编辑 → 升级为超级群）
