...
"""
会议议程内容图易拉宝合成脚本 v5
调整：①时间置顶(时间胶囊) ②完整嘉宾头衔 ③行间分隔线+交替底色 ④底部具体地点
优化：结构头压缩、小字换黑体(文泉驿正黑)更清晰、字号提升
...
OUT = 'B-易拉宝会议议程-印刷版-v5.jpg'
PREVIEW = 'B-议程预览-v5.jpg'
...
def main():
    ...
    # 字号：结构头小，正文尽量大；内容/嘉宾用黑体更清晰
    FS = dict(tag=56, day=46, sec=30, time=33, cont=40, guest=30)
    LH_CONT = int(FS['cont'] * 1.26)
    LH_GUEST = int(FS['guest'] * 1.22)
    ROW_GAP = 7
    SEC_GAP = 11
    CONT_X = PANEL_MX + 70
    CONT_MAXW = W - PANEL_MX * 2 - 70 - 50
    TIME_PAD_X = 18
    TIME_PAD_Y = 5

    def build_fonts():
        return (font(FONT_BOLD, FS['tag']), font(FONT_BOLD, FS['day']),
                font(FONT_REG, max(FS['day'] - 12, 24)), font(FONT_BOLD, FS['sec']),
                font(FONT_BOLD, FS['time']), font(FONT_HEI, FS['cont']),
                font(FONT_HEI, FS['guest']))

    f_tag, f_day, f_sub, f_sec, f_time, f_cont, f_guest = build_fonts()

    def row_h(tm, cont, guest):
        th = FS['time'] + TIME_PAD_Y * 2
    ...
    def total_h():
        return int(FS['tag'] * 1.26) + 20 + day_h(AGENDA['DAY1']) + 30 + day_h(AGENDA['DAY2'])
    ...
    for _ in range(3):
        ...
        LH_CONT = int(FS['cont'] * 1.25)
        LH_GUEST = int(FS['guest'] * 1.21)
        ROW_GAP = max(int(7 * scale), 5)
        SEC_GAP = max(int(11 * scale), 7)
        f_tag, f_day, f_sub, f_sec, f_time, f_cont, f_guest = build_fonts()
        total = total_h()
    ...
    y += int(FS['tag'] * 1.24)
    ly = y - 14
    for lx, rx in [(cx - 440, cx - 76), (cx + 76, cx + 440)]:
        d.line([(lx, ly), (rx, ly)], fill=GOLD_DEEP, width=3)
    d.line([(cx - 52, ly), (cx + 52, ly)], fill=GOLD, width=4)
    y += 2
    ...
    def draw_day(day_label, sub_label, rows):
        ...
        d.text((W - PANEL_MX - 60 - sw, y + 7), sub_label, font=f_sub, fill=BLUE_SOFT)
        y += int(FS['day'] * 1.45)
        ...
        for sec_lbl, sec_sub, items in rows:
            ...
            d.rounded_rectangle([PANEL_MX + 60, by0, PANEL_MX + 60 + lbl_w + 36, by1],
                                radius=(by1 - by0) // 2, fill=GOLD_DEEP + (210,))
            d.text((PANEL_MX + 60 + 18, by0 + int((by1 - by0 - FS['sec']) / 2) - 2),
                   lbl, font=f_sec, fill=(10, 20, 40))
            y = by1 + SEC_GAP
            for tm, cont, guest in items:
                rh = row_h(tm, cont, guest)
                if stripe % 2 == 1:
                    d.rounded_rectangle([PANEL_MX + 58, y - 6, W - PANEL_MX - 58,
                                         y + rh - ROW_GAP + 6], radius=14, fill=(32, 58, 108, 95))
                stripe += 1
                tw = d.textlength(tm, font=f_time)
                tx0, ty0 = PANEL_MX + 60, y
                ...
                y += LH_CONT * n
                if guest:
                    ng = draw_multiline(d, CONT_X, y + 2, guest, f_guest, GOLD_DIM, CONT_MAXW, LH_GUEST)
                    y += LH_GUEST * ng + 4
                y += ROW_GAP
                d.line([(PANEL_MX + 60, y - ROW_GAP // 2), (W - PANEL_MX - 60, y - ROW_GAP // 2)],
